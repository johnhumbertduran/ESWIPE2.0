...
{
    public class TrueOrFalseAnswersViewModel : ViewModelBase
    {
        #region Properties
        ...
        #region Constructor
        public TrueOrFalseAnswersViewModel()
        {
            //Title = "Teacher's Data";
            _answerService = DependencyService.Resolve<IAnswerService>();
            IsRefreshing = true;
            GetAllTrueOrFalseAnswer();
            IsBusy = IsRefreshing = false;
        }
        #endregion

        #region Methods
        private void GetAllTrueOrFalseAnswer()
        {
            IsBusy = true;
            Task.Run(async () =>
            {
                var answersList = await _answerService.GetTrueOrFalseAnswer();
        ...
        public ICommand SelectedAnswerCommand => new Xamarin.Forms.Command<AnswerModel>(async (answer) =>
        {
            if (answer != null)
            {
                ...
                {
                    await Application.Current.MainPage.Navigation.PushAsync(new TrueOrFalseAddAnswerPage(answer));
                }
